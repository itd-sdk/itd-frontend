import { A as A_1, y, u, c } from "./index-DDTOXJaD.js";
(() => {
  try {
    const e =
    ...
    if (c) {
      ...
      e._sentryDebugIds[c] = "74ca78c7-08fa-4f17-a082-af55fa6545f2";
      e._sentryDebugIdIdentifier =
        "sentry-dbid-74ca78c7-08fa-4f17-a082-af55fa6545f2";
    }
  } catch {}
})();
...
const Z = (e) => ({
  id: e.id,
  author: R(e.author),
  wallOwnerId: e.author.id,
  text: e.content,
  spans: e.spans ?? [],
  attachments: e.attachments ?? [],
  reactions: { total: e.likesCount, myReaction: null },

  stats: {
    views: e.viewsCount,
    comments: e.commentsCount,
    reposts: e.repostsCount,
  },

  reposted: false,
  originalPost: e.originalPost ? Z(e.originalPost) : null,
  dominantEmoji: null,
...
const O = [
  {
    ...
    originalPost: {
      ...
      spans: [
        { type: "hashtag", tag: "nowkie", offset: 41, length: 7 },
        { type: "hashtag", tag: "exile", offset: 49, length: 6 },
        {
          type: "hashtag",
...
итоги через 3 месяца`,
    spans: [{ type: "mention", id: "drakeoffc69", offset: 0, length: 12 }],
...
const N = 3000; /* 3e3 */
const q = 1.1;
function x({ posts, trackRef }) {
  return u("div", {
    className: o.column,
    children: u("div", {
      className: o.columnTrack,
      ref: trackRef,
      children: [0, 1].map((u) =>
        u(
          "div",
          {
            className: o.columnStack,
            children: posts.map((f, l) =>
              u(
                trackRef,
                { post: f, source: "feed_global", showcase: true },
                `${u}-${l}-${f.id}`
              )
            ),
          },
...
export const AuthLayout = ({ children }) => {
  const c = A.filter((p, s) => s % 2 === 0);

  const u = A.filter((p, s) => s % 2 === 1);
  ...
  y(() => {
    ...
    const k = (i) => {
      const r = Math.min((i - v) / 1000 /* 1e3 */, 0.05);
      v = i;
      for (let t = 0; t < 2; t++) {
        d[t] *= Math.exp(-r / q);

        if (Math.abs(d[t]) < 0.05) {
          d[t] = 0;
        }
      }
      for (let t = 0; t < 2; t++) {
        const s_t = s[t];
        const h = s_t.scrollHeight / 2;
        if (h <= 0) {
          continue;
        }
        const _ = t === 0 ? T[0] + d[0] : T[1] - d[1];
        b[t] = (((b[t] + _ * r) % h) + h) % h;
        const j = t === 0 ? b[t] - h : -b[t];
        s_t.style.transform = `translate3d(0, ${j.toFixed(2)}px, 0)`;
      }
      g = requestAnimationFrame(k);
    };

    const y = (i) => {
      const r = f_current.getBoundingClientRect();
      if (r.width === 0 || i.clientX > r.right) {
        return;
      }
      const t = s[0].getBoundingClientRect();
      const m = i.clientX <= t.right ? 0 : 1;
      d[m] = Math.max(-N, Math.min(N, d[m] - i.deltaY * W));
    };

    window.addEventListener("wheel", y, { passive: true });
    g = requestAnimationFrame(k);
  ...
  return u("div", {
    ...
    children: [
      u("div", {
        className: o.showcase,
        "aria-hidden": "true",
        ref: f,
        children: [
          u(x, { posts: c, trackRef: l }),
          u(x, { posts: u, trackRef: w }),
        ],
      }),
      u("div", { className: o.pane, children: children }),
    ],
  });
};

export { AuthLayout as AuthLayout, AuthLayout as default };
