...
import { C as Ce } from "./index-Bg1Tm6Y5.js";
import { I as rn } from "./IconInfo-DCPrcGSD.js";
import { I as dn } from "./IconNotificationMention-ChAl94AE.js";
import { I as hn } from "./IconChevronRight-CHGOa5Tt.js";
import { I as un } from "./IconChevronLeft-4Cj0xkqD.js";
(() => {
  try {
    const t =
    ...
    if (a) {
      ...
      t._sentryDebugIds[a] = "db9495e6-13d0-4c9c-9f9b-5607d90e4ab4";
      t._sentryDebugIdIdentifier =
        "sentry-dbid-db9495e6-13d0-4c9c-9f9b-5607d90e4ab4";
    }
  } catch {}
})();
    ...
    children: [
      label &&
        u("label", {
          ...
          children: [
            label,
            hint && u("span", { className: Y.hint, children: hint }),
          ],
        }),
      u("input", {
        className: `${Y.input} ${Y[h]} ${Y[y]} ${error ? Y.error : ""} ${
          className || ""
        }`,
        value: value,
        onInput: f,
      ...
      error && u("span", { className: Y.errorText, children: error }),
    ],
  });
}
const Dn = "PhGi";
...
const Zn = "Kzau";
const jn = "TspX";
const he = { content: qn, title: Vn, subtitle: Zn, actions: jn };
function Gn({ onClose }) {
  const a = async () => {
...
const fs = an_1(({ onDirtyChange, onSavingChange, onClose }, s) => {
  ...
  const M = q(
    (N, L) => {
      ...
      const Z = (L?.slug ?? null) !== (q.current?.slug ?? null);
      return U || Z;
    },
    [$]
  );
  ...
  const be = async () => {
    ...
    try {
      ...
      if ((O?.slug ?? null) !== (q.current?.slug ?? null)) {
        if (O) {
      ...
      if (x(U)) {
        if (U.errors) {
          ...
          for (const [ee, le] of Object.entries(U.errors)) {
            Z[ee] = am(le[0] || "Ошибка валидации");
          }
          v(Z);
        } else {
          A(ah(U.code, U.message || "Не удалось сохранить изменения"));
        }
      } else {
...
function As() {
  ...
  y(() => {
    ...
    function h() {
      ...
      l.current.yOff = 357 * j - T + (P ? 60 : 0);
      const i = 260;
      l.current.xOff = $s * j - S - i;
      ...
      if (C - l_current.ts >= Es) {
        ...
        if (!k) {
          ...
          a_current.style.cssText = `visibility:visible;width:${b}px;height:${A}px;background-image:url(/assets/nuksta/nuksta-chechik-sprite.png);background-repeat:no-repeat;image-rendering:pixelated;will-change:transform;background-position:${-(
            (i * j + 0.5) |
            0
          )}px 0px;background-size:${(Ps * j + 0.5) | 0}px ${
            (xs * j + 0.5) | 0
          }px;transform:translate(${(S * j - l_current.xOff + 0.5) | 0}px,${
            (P * j - l_current.yOff + 0.5) | 0
          }px)`;
        }
...
const Aa = an_1(({ onDirtyChange, onSavingChange }, p) => {
  ...
  const i = (b, A) => {
    ...
    if (f) {
      const F = Object.keys($).some((c) => $[c] !== f[c]);
      w(F);
    }
  };
...
const Da = an_1(({ onDirtyChange, onSavingChange }, p) => {
  ...
  const q = (M, E) => {
    ...
    if (y) {
      const be = Object.keys(J).some((de) => J[de] !== y[de]);
      f(be);
    }
  };
            ...
            u("div", {
              ...
              children: [
                ...
                u(ke, {
                  ...
                  options: Re,
                  onChange: (M) => q("whoCanPostOnWall", M),
                }),
              ],
            }),
            u("div", {
              ...
              children: [
                ...
                u(ke, {
                  ...
                  options: Re,
                  onChange: (M) => q("whoCanSeeMyPostReactions", M),
                }),
              ],
            }),
            u("div", {
...
function Fa({ onClose }) {
  ...
  if (y) {
    return u(Fn, { onClose: onClose, onBack: () => o(false) });
  }
  const x = S
    ? u("div", {
        className: n.actionBar,
        children: [
          u(B_1, { variant: "secondary", onClick: F, children: "Отмена" }),
          u(B_1, {
            variant: "primary",
            onClick: v,
            disabled: P,
