...
(() => {
  try {
    ...
    if (f) {
      ...
      r._sentryDebugIds[f] = "fb12ab5c-6064-41b9-93f5-5a3b1a38b3bd";
      r._sentryDebugIdIdentifier =
        "sentry-dbid-fb12ab5c-6064-41b9-93f5-5a3b1a38b3bd";
    }
  } catch {}
})();
...
function Ut(r, f, B) {
  ...
  const v = q(
    (b = Infinity) => {
      ...
      const M = Math.max(K, Math.min(E, C, b));
  ...
  const y = q(() => {
    ...
    l(C, i(w_current, C, N.current, D, L));
  }, [r, l, i]);
...
export function DrawingCanvas({
  ...
  const ot = q(() => {
    ...
    if (M_current) {
      ...
      if (M_current.tool === "line") {
        ...
        e.arc(M_current.start.x, M_current.start.y, G, 0, Math.PI * 2);
      }
  ...
  const { saveToHistory, undo, redo, canUndo, canRedo, initHistory } = qt(
    v,
    y,
    U
  );
  ...
  y(() => {
    ...
    const h = (a) => {
      if (a.touches.length === 2 && pinchRef.current) {
        ...
        const q = (g.clientY + s.clientY) / 2 - u.top;
        const H = G / pinchRef.current.dist;
        const Z_current = zoomRef.current;
        const lt = Math.min(Q, Math.max(K, Z_current * H));
        const ft = at - pinchRef.current.midX;
        const tt = q - pinchRef.current.midY;
        ...
        pinchRef.current = { dist: G, midX: at, midY: q };
      }
    };
                ...
                u("div", {
                  ...
                  children: [
                    ...
                    u("button", {
                      ...
                      disabled: R,
                      children: R
                        ? u(d_1_1, { children: [u(as, {}), "Загрузка..."] })
                        : saveButtonText ||
                          (mode === "banner"
                            ? "Загрузить баннер"
                            : "Добавить рисунок"),
