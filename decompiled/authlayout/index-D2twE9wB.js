import { A as A_1, h, u, c } from "./index-B74jCtUp.js";
(() => {
  try {
    const e =
    ...
    if (c) {
      ...
      e._sentryDebugIds[c] = "bd094820-c71d-4c0d-9f32-561d24797a93";
      e._sentryDebugIdIdentifier =
        "sentry-dbid-bd094820-c71d-4c0d-9f32-561d24797a93";
    }
  } catch {}
})();
...
export const AuthLayout = ({ children }) => {
  const c = A.filter((p, d) => d % 2 === 0);

  const u = A.filter((p, d) => d % 2 === 1);
  ...
  h(() => {
    ...
    const k = (i) => {
      ...
      for (let t = 0; t < 2; t++) {
        ...
        if (Math.abs(s[t]) < 0.05) {
          s[t] = 0;
        }
      }
      for (let t = 0; t < 2; t++) {
        const d_t = d[t];
        const h = d_t.scrollHeight / 2;
        if (h <= 0) {
          continue;
        }
        const _ = t === 0 ? T[0] + s[0] : T[1] - s[1];
        b[t] = (((b[t] + _ * f) % h) + h) % h;
        const j = t === 0 ? b[t] - h : -b[t];
        d_t.style.transform = `translate3d(0, ${j.toFixed(2)}px, 0)`;
      }
    ...
    const y = (i) => {
      ...
      const t = d[0].getBoundingClientRect();
      const m = i.clientX <= t.right ? 0 : 1;
      s[m] = Math.max(-N, Math.min(N, s[m] - i.deltaY * W));
    };
