import { a, d, A, q as q_1, ar, w, y, $, M, m, ao } from "./index-DuQT229k.js";
...
(() => {
  try {
    ...
    if (f) {
      ...
      r._sentryDebugIds[f] = "6eee9b2d-7882-45fa-ae2a-f93f8cb1678f";
      r._sentryDebugIdIdentifier =
        "sentry-dbid-6eee9b2d-7882-45fa-ae2a-f93f8cb1678f";
    }
  } catch {}
})();
...
function Ut(r, f, B) {
  ...
  const v = q_1(
    (b = Infinity) => {
      ...
      const M = 32;
      const I = (F.width - M * 2) / f;
      const C = (F.height - M * 2) / B;
      const L = Math.max(W, Math.min(I, C, b));
  ...
  const y = q_1(() => {
    ...
    const C = Math.min(q, w_current + Q);
  ...
  y(() => {
    ...
    const t = (o) => {
      ...
      if ((o.ctrlKey || o.metaKey) && (o.key === "=" || o.key === "+")) {
        ...
        const a = h.height / 2;
        const Z_current = zoomRef.current;
        const g = Math.min(q, Z_current + Q);
        updateTransform(
          g,
      ...
      if ((o.ctrlKey || o.metaKey) && o.key === "-") {
        ...
        const a = h.height / 2;
        const Z_current = zoomRef.current;
        const g = Math.max(W, Z_current - Q);
        updateTransform(
          g,
          zoomToPoint(Z_current, g, panOffsetRef.current, m, a)
        );
  ...
  y(() => {
    ...
    const o = (e) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        const s = Math.min(q, Math.max(W, Z_current + g));
        if (s === Z_current) {
          return;
        }
        updateTransform(
  ...
  y(() => {
    ...
    const h = (a) => {
      if (a.touches.length === 2 && pinchRef.current) {
        ...
        const Z_current = zoomRef.current;
        const lt = Math.min(q, Math.max(W, Z_current * J));
