import { u, d, A, q as q_1, aD, y, h, $, M, S, aA } from "./index-B74jCtUp.js";
...
(() => {
  try {
    ...
    if (f) {
      ...
      r._sentryDebugIds[f] = "0c724ca3-b07c-4c50-81d2-6136aec07c43";
      r._sentryDebugIdIdentifier =
        "sentry-dbid-0c724ca3-b07c-4c50-81d2-6136aec07c43";
    }
  } catch {}
})();
...
function Ut(r, f, B) {
  ...
  const v = q_1(
    (b = Infinity) => {
      ...
      const L = 32;
      const E = (N.width - L * 2) / f;
      const C = (N.height - L * 2) / B;
      const M = Math.max(O, Math.min(E, C, b));
      l(M, { x: (N.width - f * M) / 2, y: (N.height - B * M) / 2 });
    },
    [f, B, l, r]
  );
  ...
  h(() => {
    ...
    const h = (a) => {
      if (a.touches.length === 2 && pinchRef.current) {
        ...
        const Z_current = zoomRef.current;
        const lt = Math.min(V, Math.max(O, Z_current * J));
                ...
                u("div", {
                  ...
                  children: [
                    ...
                    u("button", {
                      ...
                      disabled: R,
                      children: R
                        ? u(width, { children: [u(aA, {}), "Загрузка..."] })
                        : saveButtonText ||
                          (mode === "banner"
                            ? "Загрузить баннер"
                            : "Добавить рисунок"),
