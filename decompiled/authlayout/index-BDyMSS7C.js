import { A as A_1, y, u, c } from "./index-pEgBAsWz.js";
(() => {
  try {
    const e =
    ...
    if (c) {
      ...
      e._sentryDebugIds[c] = "d562e3a7-5261-4eec-ab34-aa78a2a2d911";
      e._sentryDebugIdIdentifier =
        "sentry-dbid-d562e3a7-5261-4eec-ab34-aa78a2a2d911";
    }
  } catch {}
})();
      ...
      children: [0, 1].map((u) =>
        u(
          "div",
          {
            ...
            children: posts.map((r, l) =>
              u(
                trackRef,
                { post: r, source: "feed_global", showcase: true },
                `${u}-${l}-${r.id}`
              )
            ),
          },
...
export const AuthLayout = ({ children }) => {
  ...
  y(() => {
    ...
    const k = (i) => {
      const f = Math.min((i - v) / 1000 /* 1e3 */, 0.05);
      v = i;
      for (let t = 0; t < 2; t++) {
        d[t] *= Math.exp(-f / W);
    ...
    const y = (i) => {
      ...
      if (f.width === 0 || i.clientX > f.right) {
        return;
      }
      const t = s[0].getBoundingClientRect();
      const m = i.clientX <= t.right ? 0 : 1;
      d[m] = Math.max(-N, Math.min(N, d[m] - i.deltaY * H));
    };
