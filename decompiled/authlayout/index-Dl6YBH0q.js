import { A as A_1, h, u, c } from "./index-gHxZkwFX.js";
(() => {
  try {
    const e =
    ...
    if (c) {
      ...
      e._sentryDebugIds[c] = "522e030d-d285-466f-854c-cee51df9192e";
      e._sentryDebugIdIdentifier =
        "sentry-dbid-522e030d-d285-466f-854c-cee51df9192e";
    }
  } catch {}
})();
...
export const AuthLayout = ({ children }) => {
  ...
  h(() => {
    ...
    const k = (i) => {
      ...
      for (let t = 0; t < 2; t++) {
        const s_t = s[t];
        const m = s_t.scrollHeight / 2;
        if (m <= 0) {
          continue;
        }
        const _ = t === 0 ? N[0] + d[0] : N[1] - d[1];
        b[t] = (((b[t] + _ * f) % m) + m) % m;
        const j = t === 0 ? b[t] - m : -b[t];
    ...
    const y = (i) => {
      ...
      const h = i.clientX <= t.right ? 0 : 1;
      d[h] = Math.max(-T, Math.min(T, d[h] - i.deltaY * W));
    };
