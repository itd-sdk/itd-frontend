...
import { C as Ce } from "./index-CnQFC_j1.js";
import { I as rn } from "./IconInfo-CYrt_hzO.js";
import { I as dn } from "./IconNotificationMention-Bf77hpUd.js";
import { I as hn } from "./IconChevronRight-s5obFhYp.js";
import { I as un } from "./IconChevronLeft-CtO78HWI.js";
(() => {
  try {
    const t =
    ...
    if (a) {
      ...
      t._sentryDebugIds[a] = "0c7df1e0-3031-4bd8-aed6-08febf7fff27";
      t._sentryDebugIdIdentifier =
        "sentry-dbid-0c7df1e0-3031-4bd8-aed6-08febf7fff27";
    }
  } catch {}
})();
...
const fs = ao(({ onDirtyChange, onSavingChange, onClose }, s) => {
  ...
  const M = q_1(
    (N, L) => {
      ...
      const F = (L?.slug ?? null) !== (Z.current?.slug ?? null);
      return U || F;
    },
    [$]
  );
    ...
    try {
      ...
      if (x(U)) {
        if (U.errors) {
          ...
          D(ai(U.code, U.message || "Не удалось сохранить изменения"));
        }
      } else {
...
const Da = ao(({ onDirtyChange, onSavingChange }, p) => {
...
const Aa = ao(({ onDirtyChange, onSavingChange }, p) => {
  ...
  const Z = (M, E) => {
    ...
    if (y) {
      const be = Object.keys(X).some((de) => X[de] !== y[de]);
      f(be);
    }
  };
...
function Va({ onClose }) {
  ...
  const c = () => {
    switch (l) {
      ...
      case "security": {
        return u(Ba, { onChangePassword: () => o(true) });
      }
      case "notifications": {
        return u(Da, {
          ref: k,
