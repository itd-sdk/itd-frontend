import { a, d, A, q, ar, w, y, $, M, m, an as an_1 } from "./index-BAlLDjSl.js";
...
(() => {
  try {
    ...
    if (f) {
      ...
      r._sentryDebugIds[f] = "0ecd3410-873f-44e2-a666-d4bb820cbab6";
      r._sentryDebugIdIdentifier =
        "sentry-dbid-0ecd3410-873f-44e2-a666-d4bb820cbab6";
    }
  } catch {}
})();
...
function Ut(r, f, B) {
  ...
  const v = q(
    (b = Infinity) => {
      ...
      const M = 32;
      const D = (F.width - M * 2) / f;
      const C = (F.height - M * 2) / B;
      const L = Math.max(K, Math.min(D, C, b));
      l(L, { x: (F.width - f * L) / 2, y: (F.height - B * L) / 2 });
    },
    [f, B, l, r]
  );
  ...
  const y = q(() => {
    ...
    l(C, i(w_current, C, R.current, F, M));
  }, [r, l, i]);
...
export function DrawingCanvas({
  ...
  const ot = q(() => {
    ...
    const P_current = panOffsetRef.current;
    e.save();
    e.setTransform(d_current, 0, 0, d_current, 0, 0);
    e.clearRect(0, 0, m, a);
    e.translate(P_current.x, P_current.y);
    ...
    if (L_current) {
      ...
      if (L_current.tool === "line") {
        ...
        e.arc(L_current.start.x, L_current.start.y, H, 0, Math.PI * 2);
      }
  ...
  y(() => {
    ...
    const t = (o) => {
      ...
      if ((o.ctrlKey || o.metaKey) && o.key === "-") {
        ...
        const a = h.height / 2;
        const X_current = zoomRef.current;
        const g = Math.max(K, X_current - V);
        updateTransform(
          g,
          zoomToPoint(X_current, g, panOffsetRef.current, m, a)
        );
  ...
  y(() => {
    ...
    const o = (e) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        const s = Math.min(Q, Math.max(K, X_current + g));
        if (s === X_current) {
          return;
        }
        updateTransform(
  ...
  y(() => {
    ...
    const h = (a) => {
      if (a.touches.length === 2 && pinchRef.current) {
        ...
        const q = H / pinchRef.current.dist;
        const X_current = zoomRef.current;
        const lt = Math.min(Q, Math.max(K, X_current * q));
        ...
        pinchRef.current = { dist: H, midX: at, midY: G };
      }
    };
