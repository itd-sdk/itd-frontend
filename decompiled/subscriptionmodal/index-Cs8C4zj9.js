...
import { I } from "./IconNotificationMention-e79ywpKW.js";
import { I as I_1 } from "./IconInfo-CcKxyY8p.js";
(() => {
  try {
    const s =
      typeof window !== "undefined"
        ...
        ? self
        : {};
    s.SENTRY_RELEASE = { id: "1.1.0" };
    const c = new s.Error().stack;

    if (c) {
      s._sentryDebugIds = s._sentryDebugIds || {};
      s._sentryDebugIds[c] = "ca8533bb-c82f-4ec4-a7c1-bb637c7bdfd3";
      s._sentryDebugIdIdentifier =
        "sentry-dbid-ca8533bb-c82f-4ec4-a7c1-bb637c7bdfd3";
    }
  } catch {}
})();
...
function kn({ onClose, onBack }) {
  ...
  const T = async (E) => {
    ...
    o(true);
    try {
      await ab.changePassword({ currentPassword: l, newPassword: t });
...
const ws = ag(({ onDirtyChange, onSavingChange, onClose }, t) => {
  const d = w((r) => r.profile);

  const v = w((r) => r.logout);
  ...
  const f = q_1(
    (r, x) => {
      ...
      const M = Object.keys(r).some((J) => r[J] !== $[J]);
  ...
  const fe = async () => {
    ...
    try {
      ...
      if (a8(M)) {
        if (M.errors) {
          const O = {};
          for (const [J, ae] of Object.entries(M.errors)) {
            O[J] = ae[0] || "Ошибка валидации";
          }
          L(O);
        } else {
          B(M.message || "Не удалось сохранить изменения");
        }
      } else {
...
const Ms = ag(({ onDirtyChange, onSavingChange }, m) => {
  const { settings, fetchSettings, updateSettings } = Z();
...
const As = ag(({ onDirtyChange, onSavingChange }, m) => {
  const [t, d] = d({
    isPrivate: false,
  ...
  const V = async () => {
    ...
    try {
      const f = await o.getPrivacySettings();
  ...
  const Z = async (f) => {
    if (!$) {
      F(true);
      try {
        const S = await x.getBlockedUsers({ cursor: f, limit: 20 });
...
const Ls = [
  { id: "account", icon: al, label: "Аккаунт" },
  { id: "appearance", icon: s, label: "Оформление" },
  { id: "security", icon: nn, label: "Безопасность" },
  { id: "privacy", icon: en, label: "Приватность" },
  { id: "notifications", icon: ak, label: "Уведомления" },
];
...
function Zs() {
  const s = B_1(null);
  const c = B_1(null);
  const l = B_1({ f: 0, ts: 0, tl: Ne.slice(), xOff: 0, yOff: 0 });
    ...
    function I(k) {
      ...
      if (k - l_current.ts >= Os) {
        ...
        if (!w) {
          ...
          const p = (C * W + 0.5) | 0;
          const B = (L * W + 0.5) | 0;
          c_current.style.cssText = `visibility:visible;width:${p}px;height:${B}px;background-image:url(/assets/nuksta/nuksta-chechik-sprite.png);background-repeat:no-repeat;image-rendering:pixelated;will-change:transform;background-position:${-(
            (h * W + 0.5) |
            0
          )}px 0px;background-size:${(Ds * W + 0.5) | 0}px ${
            (Rs * W + 0.5) | 0
