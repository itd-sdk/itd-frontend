import { d, A, y, q, a, a3, ag } from "./index-BCnJB3wz.js";
...
(() => {
  try {
    ...
    if (I) {
      ...
      c._sentryDebugIds[I] = "fe5bc8cf-bdb0-4afc-8ea6-0f376cbd545e";
      c._sentryDebugIdIdentifier =
        "sentry-dbid-fe5bc8cf-bdb0-4afc-8ea6-0f376cbd545e";
    }
  } catch {}
})();
function ve() {
  ...
  const _ = q(() => {
    ...
    const C = new Uint8Array(G_current_frequencyBinCount);
    G_current.getByteFrequencyData(C);
    let W = 0;
  ...
  const Z = q(async () => {
    try {
      ...
      a.current = w;
      const C = w.createAnalyser();
      C.fftSize = 256;
      C.smoothingTimeConstant = 0.3;
      G.current = C;
      w.createMediaStreamSource(r).connect(C);
      ...
      v.current = window.setInterval(() => {
        const J = Math.floor((Date.now() - A) / 1000 /* 1e3 */);
        R(J);
      }, 100);
        ...
        if (i.current && i.current.state !== "inactive") {
          i.current.onstop = () => {
            if (g.current.length > 0) {
              ...
              const W = new Blob(g.current, { type: C });
